	 ...
	 if (is_uploaded_file($key['tmp_name'])) {
			...
			if ($key["type"]=="image/jpeg" || $key["type"]=="image/pjpeg" || $key["type"]=="image/gif" || $key["type"]=="image/bmp" ||$key["type"]=="image/png")
						{   
							...
							$sql="UPDATE  `categoriasproductos` SET  `foto`='".$imagenEscapes."' WHERE   `id` =".$_SESSION['id'].";";
							 $mysqli->query($sql);
	   ...
	   <img id='banner'  src='".$folderAdmin."/imgcatprod.php?id=".$_SESSION['id']."&".date('is')."' onerror=\"this.src='".$folderAdmin."/sin_foto.jpg';\" style=' width: 100%;   ' >
