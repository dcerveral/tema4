...
if ($_GET['id'] )
{
		$sql="DELETE FROM `categoriasproductos`  WHERE  `id` =".$_GET['id'].";";
...
//GESTION DE SEO
	...
	$seo=amigo($_POST['titulo']);
       //Si ya existe ese seo, le insertamos otro
		...
		$consulta= "SELECT id FROM `categoriasproductos`  WHERE  `seo` ='".$seo."' ".$AND.";";
...
if ($_POST['accion']=="editar")
{
	...
	$sql="UPDATE  `categoriasproductos` SET  `categoria` =  '".$_POST['titulo']."',`seo` =  '".$seo."',  `superior` =  '".$_POST['superior']."' ,  `descripcion` =  '".$_POST['descripcion']."' WHERE
...
if ($_POST['accion']=="nueva") {				
		...
		$consulta="INSERT INTO `categoriasproductos` (`id`, `categoria`, `superior`, `foto`, `seo`, `descripcion`)
...
<section class="content-header">
      <h1>
        Catalogo <small>Categorias</small>
          ...
          <div class="box">
            <div class="box-header">
			  ...
			  <div class="box-tools pull-right">
                ...
                <a href="<? echo $folderAdmin; ?>/nuevacatprod" type="button" class="btn  btn-success btn-xs" > <i class="fa fa-plus-circle" aria-hidden="true"></i> Nueva</a>
              </div>
            </div>
            <!-- /.box-header -->
            <div class="box-body">
              <table id="example1" class="table table-bordered table-hover table-striped">
				...
				<?
				$consulta="select  id, categoria, superior, seo, descripcion  from categoriasproductos ORDER BY id DESC  ";
				...
				while($campo=mysql_fetch_array($sql)){
					...
					echo "<tr id='tr".$campo[0]."'><td  ><span style='display:none;'>".$campo[0]."</span><img src='".$folderAdmin."/imgcatprod.php?id=".$campo[0]."&".date('is')."' class=' img-thumbnail'  style='width:100px;'     onerror=\"this.src='".$folderAdmin."/sin_foto.jpg';\"></td>
					...
					if ($campo[0]==1) {
						...
						echo "<a class='btn btn-danger' style='margin:1px;' onclick=\"document.getElementById('eliminar').href='".$folderAdmin."/categorias-productos/".$campo[0]."'; $('#myModal').modal('show') ;\" ><i class='fa fa-trash'></i> </A>";
							
						}
					
					echo "<a class='btn btn-success' style='margin:1px;' href='".$folderAdmin."/editarcategoriaproductos/".$campo[0]."'><i class='fa fa-pencil-square-o'></i> </A>";
